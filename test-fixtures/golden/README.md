...
- `psycho-build` (pipeline), `cascade-words`, `validate --fix --apply`,
  `sync-timelines` (non-dry-run): need synthetic drafts / font calibration /
  mirrors. They are covered by their unit suites.
- `gc` runs only as a no-op here (the fixtures have no removable orphans); the
  destructive path's data-safety assertion is `test/validate-fix.test.mjs:255`.
