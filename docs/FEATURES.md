...
|---|---|---|
| ElevenLabs voice calibration (dry-run → approval → execute → WPM publication) | `src/calibration/` (ports & adapters), `src/commands/calibration-ui.ts`, `src/calibration-cli.ts` | [`calibration-architecture.md`](./calibration-architecture.md), [`calibration-mvp.md`](./calibration-mvp.md), [`elevenlabs-calibration-contract-inventory.md`](./elevenlabs-calibration-contract-inventory.md) |
...
- `Shared/voice-calibration/voice_wpm.json` — canonical WPM authority; local profiles
  are projections validated against it (see `src/calibration/bridge.ts`).
...
- `Shared/calibration-ui-mvp/` — linked git worktree of this repo (branch `master`), home
  of the standalone-calibration experiments and its architecture docs.
